...
# get the nodes
proteins_go_arabidopsis <- V(g_arabidopsis)$name
...
results_dict_diffml_arabidopsis <- list()
...
for (go_term in go_terms_arabidopsis){
  ...
  for (portion in seq_along(split_proteins_arabidopsis)) {
    ...
    go_vector <- setNames(ifelse(proteins_go_arabidopsis %in% seed_proteins_unselected, 1, 0), proteins_go_arabidopsis)
    ...
    results_dict_diffml_arabidopsis[[go_term]][split_proteins[[portion]]] <- output_vec
  }
}
